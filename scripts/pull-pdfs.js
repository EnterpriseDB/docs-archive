...
function pullFiles()
{
  ...
  for (let product in productStubs)
  {
    for (let version in productStubs[product])
    {
      ...
      {
        PDFs.push(...section.links.map(l => l.pdf).filter(l => l && l.endsWith(".pdf")));
      }
    }
  }
  ...
  for (let pdf of PDFs)
  {
    const destFile = pdf.replace("https://www.enterprisedb.com/edb-docs/static/", "")
      .replace("https://www.enterprisedb.com/edb-docs/d/", "docs/")
    download(pdf, destFile, (error) => 
    { 
      if (error)
        downloadedPDFs.push({error, pdf});
      else
        downloadedPDFs.push(pdf); 
      reportStatus();
    });
  }
  ...
  function reportStatus()
  {
    if (downloadedPDFs.length < PDFs.length)
    {
      ...
      const pdfDesc = `file ${downloadedPDFs.length} of ${PDFs.length}: ${last.pdf||last}`;
      if (last.error)
        console.error(`Error downloading ${pdfDesc}:
        ${last.error}`);
      else
        console.log(`downloaded ${pdfDesc}`)
    }
    else
    {
      const failed = downloadedPDFs.filter(d => d.error).length;
      console.log(`downloaded ${downloadedPDFs.length-failed} of ${downloadedPDFs.length} (${failed} failed)`)
    }
  }
}
